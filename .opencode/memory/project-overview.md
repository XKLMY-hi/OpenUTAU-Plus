...
## 近期主要工作
- **2026-09-18 环境重启后构建链重建 + 上游定向移植（A8-A10 + B1/B2）全部完成**：
  - 环境换盘（E 盘消失）→ 用系统 dotnet（SDK 10 编 net8.0）+ 用户目录离线 NuGet 重建构建链，命令与坑见 [[env-refresh-2026-09-g-drive]]
  - 钢琴窗 A8 播放音符弹跳（2954a723）· A9 悬停光晕改进（bdcaf9b4）· A10 音轨区显示范围高亮（ff2836ec）· B1 Alt 拖拽复制（b0d001c0）· B2 曲线编辑工具扩展（d3afb1ed，含 UCurve.ReplaceRange）——均上游逐行等价移植（B2 有 3 处 Plus 适配），待用户实机验收
  - 顺带修掉既有测试 flake：RenderGate 并发用例原断言硬编码全局零值，改为基线相对断言（c8532db6）；测试基线 284 → **294**；全部提交已推送 origin/plus-develop
- **2026-09-16 节拍器完成验收 + 字体崩溃加固 + 钢琴窗批次 A 过半**：
  ...
  - 钢琴窗批次 A：A1 关闭按钮/双击隐藏 b65d1f8f · A2-A5 编辑工具链 d7ac5f82 · A6 发音提示 eb5ba981 · A7 悬停辉光+播放高亮 c9a04174；**A9/A10/批次 B 待做**，详见 [[metronome-piano-port]]
